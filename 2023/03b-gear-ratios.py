...
gear_ratio_sum = 0
for symbol in symbols:
    # for part 2 we only care about gears
    if symbol["char"] != "*":
        continue

    adjacent_parts = []
    # loop through all parts and check if any are adjacent to this gear
    for part in parts:
        # check if the symbol is on the same row as the part
        if symbol["row"] == part["row"]:
            # check if the symbol is adjacent to the part
    ...
    if len(adjacent_parts) == 2:
        gear_ratio_sum += adjacent_parts[0]["part_number"] * adjacent_parts[1]["part_number"]
