...
# Now... expand the galaxies a lot.
# I wonder why there's a weird off by one error...
EXPANSION_FACTOR = 999999

for galaxy in galaxies:
    # how many empty columns appear before this galaxy?
    num_empty_columns_before = len([idx for idx in empty_column_idxs if idx < galaxy["x"]])
    # how many empty rows appear before this galaxy?
    num_empty_rows_before = len([idx for idx in empty_row_idxs if idx < galaxy["y"]])

    galaxy["x"] += num_empty_columns_before * EXPANSION_FACTOR
    galaxy["y"] += num_empty_rows_before * EXPANSION_FACTOR
